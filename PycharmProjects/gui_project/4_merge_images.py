...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
                                        ...
                                        initialdir=r"C:\Users\seonghyeon\Documents\practice\PycharmProjects\gui_project")
...
def merge_image():
    ...
    dest_path = os.path.join(txt_dest_path.get(), save_name.get() + "." + cmb_format.get())
...
def start():
    ...
    if save_name.get() == "":
        msgbox.showwarning("경고", "저장 이름을 설정해주세요.")
        return
...
save_label = Label(frame_run, text="저장 이름 : ", padx=5, pady=5)
save_label.pack(side="left", padx=5, pady=5)
save_name = Entry(frame_run, width=30)
